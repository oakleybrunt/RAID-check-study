...
# cb_data_raid = pl.read_csv("data/single_node_xios/collbuff_RAID_nomult.csv")
# nh_data_raid = pl.read_csv("data/single_node_xios/nohints_RAID_nomult.csv")
...
cb_data = cb_data.with_columns(
    ...
    Identifier = pl.lit("collective buffering hints"),
    size_GiBs = pl.col("n_bytes").truediv(1024**3)
    )
nh_data = nh_data.with_columns(
    pl.col("created").str.to_datetime(format="%Y-%m-%d %H:%M:%S%.f"),
    pl.col("raw_write_rate_mibs").cast(pl.Float64, strict=False).alias("raw_write_rate_mibs"),
    Identifier = pl.lit("no hints"),
    size_GiBs = pl.col("n_bytes").truediv(1024**3)
    )
...
plot_data = cb_data.vstack(nh_data)

plot_data = plot_data.filter(
    pl.col("created").le(min_date)
)
